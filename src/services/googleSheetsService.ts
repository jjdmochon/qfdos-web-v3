...
export async function submitAttemptToGoogleSheets(
  ...
  const payload = {
    ...
    // cuando quien envía es profesor
    sesion: tokenSesion(),
...
export async function misCalificaciones(email: string): Promise<QuizRegistrationRecord[] | null> {
  ...
  if (!sesion) return getCachedCalificaciones(norm);
  ...
  const promesa = (async (): Promise<QuizRegistrationRecord[] | null> => {
    try {
      const resp = await fetch(
        `${url}?accion=misCalificaciones&email=${encodeURIComponent(norm)}` +
...
/**
 * Código oficial de Google Apps Script listo para copiar y pegar en la hoja del
 * docente. Se importa del propio fichero .gs para que no haya dos copias que
 * puedan divergir.
 */
export const GOOGLE_APPS_SCRIPT_TEMPLATE = calificacionesGs.trim();
